...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/ferro-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    FERROD_EXEC=ferrod.exe
fi
...
get_tag
popd
echo "var GUI_FERRO_VERSION = \"$TAGNAME\"" >> version.js

cd build
qmake ../ferro-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy ferrod to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$FERRO_DIR/bin/$FERROD_EXEC $BIN_PATH
fi
