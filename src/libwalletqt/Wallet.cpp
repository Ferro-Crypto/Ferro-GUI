...
class WalletListenerImpl : public  Ferro::WalletListener
{
public:
...
{
    QFuture<Ferro::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Ferro::Wallet::connected);
    QFutureWatcher<Ferro::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Ferro::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Ferro::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Ferro::Wallet::ConnectionStatus> future = connectionWatcher->future();
...
{
    Ferro::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Ferro::PendingTransaction::Priority>(priority));
...
{
    Ferro::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Ferro::optional<uint64_t>(), mixin_count,
                static_cast<Ferro::PendingTransaction::Priority>(priority));
...
{
    Ferro::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Ferro::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Ferro::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitferro.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/ferro-wallet-gui.log";
}

Wallet::Wallet(Ferro::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_history;
    m_history = NULL;
    //Ferro::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
    delete m_walletImpl;
    m_walletImpl = NULL;
