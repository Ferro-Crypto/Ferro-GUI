...
{
    // prepare command line arguments and pass to ferrod
    ...
    qDebug() << "starting ferrod " + m_ferrod;
    ...
    // Start ferrod
    bool started = m_daemon->startDetached(m_ferrod, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM ferrod.exe");
#else
                QProcess::execute("pkill ferrod");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./ferrod status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to ferrod
#ifdef Q_OS_WIN
    m_ferrod = QApplication::applicationDirPath() + "/ferrod.exe";
#elif defined(Q_OS_UNIX)
    m_ferrod = QApplication::applicationDirPath() + "/ferrod";
#endif

    if (m_ferrod.length() == 0) {
