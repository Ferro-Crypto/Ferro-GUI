; Ferro Helium Hydra GUI Wallet Installer for Windows
; Copyright (c) 2014-2017, The Ferro Project
; See LICENSE

[Setup]
AppName=Ferro GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\Ferro GUI Wallet
DefaultGroupName=Ferro GUI Wallet
UninstallDisplayIcon={app}\ferro-wallet-gui.exe
...
; Ferro GUI wallet
Source: "bin\ferro-wallet-gui.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ferro GUI wallet log file
...
Source: "ferro-wallet-gui.log"; DestDir: "{app}"; Flags: comparetimestamp; Permissions: users-modify

; Ferro CLI wallet
Source: "bin\ferro-wallet-cli.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ferro wallet RPC interface implementation
Source: "bin\ferro-wallet-rpc.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ferro daemon
Source: "bin\ferrod.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ferro daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "ferro-daemon.bat"; DestDir: "{app}"; Flags: comparetimestamp;

; Ferro blockchain utilities
Source: "bin\ferro-blockchain-export.exe"; DestDir: "{app}"; Flags: comparetimestamp
Source: "bin\ferro-blockchain-import.exe"; DestDir: "{app}"; Flags: comparetimestamp

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\ferro-utils-deserialize.exe"; DestDir: "{app}"; Flags: comparetimestamp
...
; Filename: "{app}\ferro-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitferro');
  s := 'The default folder to store the Ferro blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bitferro.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "ferro-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Ferro daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\ferrod.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\ferro-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Ferro GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\ferro-wallet-gui.exe"
...
Name: "{group}\Utilities\Ferro Daemon"; Filename: "{app}\ferrod.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\ferro-wallet-cli.exe"; WorkingDir: "{userdocs}\Ferro\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Ferro\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\ferro-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\ferro-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM ferrod.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\ferro-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\ferro-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\ferro-project\ferro-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\ferro-project\ferro-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
