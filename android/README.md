...
# Base environnement
        ...
        cd ferro/utils/build_scripts
        docker build -f android32.Dockerfile -t ferro-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t ferro-gui-android .
        docker create -it --name ferro-gui-android ferro-gui-android bash

# Get the apk

        docker cp ferro-gui-android:/opt/android/ferro-core/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/ferro-core/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
